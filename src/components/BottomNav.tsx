...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-200 px-4 pt-3 pb-8 flex justify-between items-center z-40">
      {navItems.map((item) => (
        <Link
          key={item.href}
          href={item.href}
        ...
        >
          ...
          <span className={`text-[8px] ${item.active ? "font-bold" : "font-medium"}`}>
            {item.label}
          </span>
